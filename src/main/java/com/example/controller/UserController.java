...
public class UserController {
    ...
    public String index(Model model) {
        List<Products> result = productsService.list(new QueryWrapper<Products>().eq("product_is_delete", false));
    ...
    public String queryCollection(Model model, @AuthenticationPrincipal UserDetails userDetails) {
        Users users = usersService.getOne(new QueryWrapper<Users>().eq("username", userDetails.getUsername()));
        String[] strList = users.getCollection().split(",");
        List<Products> result1 = productsService.list(new QueryWrapper<Products>().in("product_id", strList).eq("product_category", 1).eq("product_is_delete", false));
        List<Products> result2 = productsService.list(new QueryWrapper<Products>().in("product_id", strList).eq("product_category", 2).eq("product_is_delete", false));
    ...
    @ResponseBody
    public Result generateOrder(String productId, String productAmount, Boolean isGroup, @AuthenticationPrincipal UserDetails userDetails) {
        ...
        Orders orderResult = ordersService.generateOrder(productId_m, productAmount_m, isGroup, one.getUserId());
